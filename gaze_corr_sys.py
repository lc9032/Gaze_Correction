...
class GazeCorrSys():
    ...
    def run(self):
        ...
        if CAMERA_VIDEO_SWITCH == 1:
            ...
            # ffmpeg.input(input_video_path).output('./audio.aac').run(overwrite_output=True)
            # ffmpeg.input(output_video_path).output('./audio.aac').output(output_video_path, vcodec='copy', acodec='aac').run(overwrite_output=True)
