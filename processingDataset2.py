...
def extract_headpose_from_xml(root):
    ...
    for element in root.findall(".//HeadposeDef"):
        ...
        if element is not None:
            ...
            points.append((int(float(pose_x)), int(float(pose_y)), int(float(gaze_x)), int(float(gaze_y))))
...
def eye_landmarks(points, eye_region, x, y, w, h):
    ...
    for i in key_indices:
        ...
        # cv2.circle(eye_region, resize_point, 2, (255, 0, 255), -1)
    ...
    for p1, p2 in center_pairs:
        ...
        # cv2.circle(eye_region, resize_center_point, 2, (255, 0, 255), -1)
...
def plot_points_on_image(user_number, dataset_folder, xml_path, pose_xml_path, output_path):
    ...
    # for image_number in range(1,2):
        ...
        pose = poseGaze[image_number-1][0][1]
        gaze_y = poseGaze[image_number-1][0][2]
        gaze_x = -poseGaze[image_number-1][0][3]
        ...
        l_left_most = interior_margin_points[image_number-1+pointsGrids][0]
        ...
        cv2.imwrite(output_path + f"right/{user_number:04}_2m_{pose}P_{gaze_x}V_{gaze_y}H.jpg", right_eye_region)
        print(f"Output saved to {output_path}")

        cv2.imwrite(output_path + f"left/{user_number:04}_2m_{pose}P_{gaze_x}V_{gaze_y}H.jpg", left_eye_region)
        print(f"Output saved to {output_path}")

        # Save the landmarks to a text file
        output_path_info = os.path.join(output_path, f"info/right/{user_number:04}_2m_{pose}P_{gaze_x}V_{gaze_y}H.txt")
        with open(output_path_info, "w") as f:
            # f.write("Left Eye Landmarks:\n")
            for point in r_info_points:
                f.write(f"{point[0]},{point[1]}\n")

        output_path_info = os.path.join(output_path, f"info/left/{user_number:04}_2m_{pose}P_{gaze_x}V_{gaze_y}H.txt")
        with open(output_path_info, "w") as f:
            # f.write("\nRight Eye Landmarks:\n")
...
for user_number in range(1, users+1):
    ...
    for hp_number in range(1, 126):
        ...
        output_path = f"./DataSets/preprocessing_dataset_U2/{user_number:04}/"
