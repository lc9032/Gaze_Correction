...
class Transformation:
    ...
    def interpolate(self, image, x, y, output_size):
    ...
    def replace_zeros_with_ones(self, transformed_image):
        epsilon = 1e-2  # Threshold to consider a value as zero
        mask = tf.reduce_all(tf.less_equal(tf.abs(transformed_image), epsilon), axis=-1, keepdims=True)
        mask = tf.cast(mask, tf.float32)
        ones = tf.ones_like(transformed_image)
        return transformed_image * (1 - mask) + ones * mask

    def apply_transformation(self, flows, img):
        with tf.name_scope("apply_transformation"):
            ...
            indices_grid = self.meshgrid(height, width)
            ...
            transformed_image = self.interpolate(img, x_s_flatten, y_s_flatten, (height, width))
            ...
            # transformed_image = self.replace_zeros_with_ones(transformed_image)
